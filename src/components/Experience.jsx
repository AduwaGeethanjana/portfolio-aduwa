        ...
        <div>
            {EXPERIENCES.map((experience, index) => (
              <div key={index} className="mb-8 flex flex-wrap lg:justify-center">
                ...
                <p className="mb-2 text-sm text-neutral-400 text-justify">{experience.year}</p>  
                </motion.div>
                <motion.div
                whileInView={{ opacity: 1, x: 0}}
                initial={{ opacity: 0, x: 100}}
            ...
            {experience.project_3 && (
                ...
                {experience.technologies.map((tech, index) => (
                    <span
                    key={index}
                    className="mr-2 mt-4 rounded bg-neutral-900 px-2 py-2 py-1 text-sm font-medium text-purple-800">{tech}
                    </span>
                )
                )}
                </motion.div>
